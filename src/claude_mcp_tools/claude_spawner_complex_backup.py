...
def spawn_claude_sync(
    ...
    try:
        ...
        # Build claude command - use shared MCP config that connects to existing server
        shared_mcp_config = {
            "mcpServers": {
                "claude-mcp-orchestration": {
                    "command": "/home/zach/.mcptools/start-orchestration.sh",
                    "transport": {
                        "type": "stdio"
                    }
                }
        ...
        # Add output format for proper process handling
        ...
        if enable_logging:
            ...
            # Create cross-platform log file path
            log_file_path = str(log_dir / f"claude_agent_{process.pid}_{timestamp}.log")
...
def monitor_agent_output(process: subprocess.Popen, log_file_path: str | None = None) -> Generator[str, None, None]:
    ...
    try:
        ...
        # Ensure process is properly reaped to prevent zombies
        try:
            if process.poll() is None:  # Process still running
                process.wait(timeout=5.0)  # Give it time to finish
            else:
                process.wait()  # Reap finished process
            logger.info(f"Process {process.pid} properly reaped with return code: {process.returncode}")
        except subprocess.TimeoutExpired:
            logger.warning(f"Process {process.pid} timed out, force killing")
            process.kill()
            process.wait()  # Force reap after kill
        except Exception as e:
            logger.error(f"Error reaping process {process.pid}: {e}")
            try:
                process.kill()
                process.wait()
            except:
